...
from copaw_worker.user_paths import bind_human_id, ensure_user_workspace
    ...
    async def query_handler(
        ...
        **kwargs,
        ...
        try:
            ...
            base_workspace_dir = self.workspace_dir if self.workspace_dir else WORKING_DIR
            ...
            with bind_human_id(user_id):
                user_workspace_dir = ensure_user_workspace(
                    base_workspace_dir,
                    user_id,
                )
                ...
                if (
                    ...
                    and hasattr(self.memory_manager, "prepare_for_user")
                ):
                    await self.memory_manager.prepare_for_user()

                env_context = build_env_context(
                    session_id=session_id,
                    user_id=user_id,
                    channel=channel,
                    working_dir=str(user_workspace_dir),
                )
                ...
                try:
                    ...
                    logger.warning(
                        "load_session_state skipped (state schema mismatch): "
                        "%s; will save fresh state on completion to recover "
                        "file",
                        e,
                    )
